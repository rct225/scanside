...
/**
 * pci1710_nofifo_isr - interrupt service routine for without fifo
 *                      data acquisition
 *
 ...
 */
static void pci1710_nofifo_isr(private_data *ptr)
{
...
/**
 * pci1710_fifo_isr - interrupt service routine for without fifo
 *                    data acquisition
 *
 * @ptr: point to the private data of device object
 *
 ...
 */
static void pci1710_fifo_isr(private_data *ptr)
{
	private_data *privdata = ptr;
	adv_user_page *page = NULL;
	INT16U tmp;
	...
	if (privdata->cur_index == privdata->conv_num / 2) {
		privdata->transfered += (privdata->conv_num / 2);
		/* privdata->retrieved = privdata->cur_index; */
		privdata->half_ready = 1;
		adv_process_info_set_event_all(&privdata->ptr_process_info, 1, 1);
...
/**
 * pci1710_interrupt_handler - PCI1710 interrupt handler                    
 *
 *                                                            
...
#endif
{
	...
	if (tmp) {
		if (privdata->fifo_enable) {
			pci1710_fifo_isr(privdata);
		} else {
			pci1710_nofifo_isr(privdata);
		}
