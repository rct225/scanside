...
INT32S adv_ioctrl(adv_device *device, unsigned int cmd, unsigned long arg)
{
	...
	case ADV_ENABLE_EVENT:
		...
		ret = adv_enable_event(device, lpEnableEvent.EventType,
				       lpEnableEvent.Enabled, lpEnableEvent.Count);
		break;
...
/* PCI-1752 driver object */
adv_driver pci1752_driver =
{
     .driver_name		= "pci1752",
     .devfs_name		= "pci1752",
     .ops			= &dev1752_fops,
...
{     
	...
	if (request_region(privdata->iobase, privdata->iolength, "PCI-1752") == NULL) {
		kfree(device);
		kfree(privdata);
		return -EFAULT;
	}
	
	if (request_irq(privdata->irq, pci1752_interrupt_handler, SA_SHIRQ, "adv1752", privdata)) {
		kfree(device);
		kfree(privdata);
		return -EFAULT;
	}
	...
	adv_disable_device_freeze(privdata);
	...
	case PCI1752:
		...
		case SUB_PCI1752:
			advdrv_device_set_devname(device, "pci1752");
			break;
		case SUB_PCI1752U:
			advdrv_device_set_devname(device, "pci1752u");
			break;
		case SUB_PCI1752USO:
			advdrv_device_set_devname(device, "pci1752uso");
			break;
		}
		break;
	case PCI1754:
		advdrv_device_set_devname(device, "pci1754");
		break;
	case PCI1756:
		advdrv_device_set_devname(device, "pci1756");
		break;
	default:
		break;
	...
	advdrv_add_device(&pci1752_driver, device);
...
static void __devexit advdrv_remove_one(struct pci_dev *dev)
{
	...
	advdrv_remove_device(&pci1752_driver, device);
...
/************************************************************************
 * static int __init pci1752_init(void)
 * 
 * Description:  The module initialize PCI-1752 driver - 
...
static int __init pci1752_init(void)
{
     ...
     //pci1752_driver.driver_sema = sema;
     _ADV_SET_DRIVER_SEMA(pci1752_driver, sema);
     
     advdrv_register_driver( &pci1752_driver );
     error =  pci_module_init(&pci_driver) ;
     if( error != 0 )
     {
	  advdrv_unregister_driver( &pci1752_driver );
	  kfree(sema);
	  return error;
     }
     return SUCCESS;
 ...
 * static int __init pci1752_init(void)
 * 
 * Description:  Exit  PCI-1752 driver .
 *************************************************************************/
static void __exit pci1752_exit(void)
{
     struct semaphore *sema = NULL;
     pci_unregister_driver( &pci_driver );
     advdrv_unregister_driver( &pci1752_driver );
     
     _ADV_GET_DRIVER_SEMA(pci1752_driver, sema);
     
     kfree(sema);
}

module_init( pci1752_init );
module_exit( pci1752_exit );

MODULE_DEVICE_TABLE(pci, serial_pci_tbl);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Advantech PCI1752 device driver module");
