...
INT32S adv_read_port(adv_device *device, void *arg)
{
	...
	case AD_READ_PORT_UCHAR:
		*(UCHAR *) buf = advInp(privdata, port_num);
		ret_len = sizeof(UCHAR);
		break;
	case AD_READ_PORT_USHORT:
		*(USHORT *) buf = advInpw(privdata, port_num);
		ret_len = sizeof(USHORT);
		break;
...
INT32S adv_write_port(adv_device *device, void *arg)
{
	...
	case AD_WRITE_PORT_UCHAR:
		advOutp(privdata, port_num, *(UCHAR *) buf);
		ret_len = sizeof(UCHAR);
		break;
	case AD_WRITE_PORT_USHORT:
		advOutpw(privdata, port_num, *(USHORT *) buf);
		ret_len = sizeof(USHORT);
		break;
