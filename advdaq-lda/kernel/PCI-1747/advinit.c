 ...
 *			Advantech Co., Ltd.                       
 *                                                                         
 *	Advantech PCI-1747 Series Device driver for Linux
 *		
 * (pci1747, pci1747L, pci1747HG, pci1747HGL, pci1711, pci1711L, pci1741U)
 *                                                                         
 *                                                                         
 * File Name:                                                        
 * 	advinit.c                                                 
 * Abstract:                                                         
 * 	Initialize the PCI-1747 driver module and device.         
 *                                                                         
 * Version history		                                          
...
{
	{       0x13fe, 0x1747,	/* PCI-1747 */
		0x13fe, 0xa100,
		0, 0, 0
	},
	{       0x13fe, 0x3747,	/* MIC-3747 */
		0x13fe, 0x0000,
		0, 0, 0
	},
	{0},
...
/* PCI-1747 driver object */
adv_driver pci1747_driver = {
	.driver_name	= "pci1747",
	.devfs_name	= "pci1747",
	.ops		= &dev1747_fops,
};
...
{     
	...
	privdata->sem_fai = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
	if (privdata->sem_fai == NULL) {
		kfree(device);
		kfree(privdata);
		return -ENOMEM;
	}
	init_MUTEX(privdata->sem_fai);
	...
	/* request I/O regions */
	if (request_region(privdata->ioDMAbase, privdata->ioDMAlength, "PCI-1747") == NULL) {
		kfree(device);
		kfree(privdata);
	...
	if (request_region(privdata->iobase, privdata->iolength, "PCI-1747") == NULL) {
	...
	/* request irq */
	ret = request_irq(privdata->irq, pci1747_interrupt_handler, SA_SHIRQ, "adv1747", privdata);
	if (ret != 0) {
		release_region(privdata->ioDMAbase, privdata->ioDMAlength);
	...
	case PCI1747:
		...
		advdrv_device_set_devname(device, "pci1747");
		break;
	case MIC3747:
		privdata->board_id = (INT16U) (advInp(privdata, 0x20) & 0x0f);
		advdrv_device_set_devname(device, "mic3747");
		break;
	default:
		release_region(privdata->ioDMAbase, privdata->ioDMAlength);
		release_region(privdata->iobase, privdata->iolength);
	...
	ret = advdrv_add_device(&pci1747_driver, device);
	if (ret) {
		release_region(privdata->ioDMAbase, privdata->ioDMAlength);
		release_region(privdata->iobase, privdata->iolength);
 		free_irq(privdata->irq, privdata);
...
static void __devexit advdrv_remove_one(struct pci_dev *dev)
{
	...
	advdrv_remove_device(&pci1747_driver, device);
...
/**
 * pci1747_init - The module initialize PCI-1747 driver
 *
 * Enumerate all PCI cards on the bus, register the driver in Advantech core driver.
 */
static INT32S __init pci1747_init(void)
{
	INT32S error;
	...
	init_MUTEX(sema);
	_ADV_SET_DRIVER_SEMA(pci1747_driver, sema);


	advdrv_register_driver(&pci1747_driver);
	error = pci_module_init(&pci_driver);
	if (error) {
		advdrv_unregister_driver(&pci1747_driver);
		return error;
	}
...
/**
 * pci1747_exit - Exit PCI-1747 driver
 */
static void __exit pci1747_exit(void)
{
	struct semaphore *sema = NULL;
	...
	advdrv_unregister_driver( &pci1747_driver );
	_ADV_GET_DRIVER_SEMA(pci1747_driver, sema);
     
	kfree(sema);
}

module_init(pci1747_init);
module_exit(pci1747_exit);

MODULE_DEVICE_TABLE(pci, serial_pci_tbl);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Advantech PCI1747 device driver module");
