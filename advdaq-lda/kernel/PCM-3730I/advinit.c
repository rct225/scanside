...
adv_driver pcm3730i_driver = {
	.owner = THIS_MODULE,
	.driver_name		= "pcm3730i",
	.devfs_name		= "pcm3730i",
	.ops			= &dev3730i_fops,
};
...
{     
	...
	if (request_region(privdata->iobase, privdata->iolength, "PCI-3730i" ) == NULL) {
	...
	if (request_irq(privdata->irq, pcm3730i_interrupt_handler, SA_SHIRQ, "adv3730i", privdata)) {
	...
	{
	case PCM3730I:
		advdrv_device_set_devname(device, "pcm3730i");
		break;
	default:
		return -1;
	  
	}
	advdrv_add_device( &pcm3730i_driver, device );
	printk("Add a Advantech PCM%x device:boardID=%d;iobase=0x%lx;irq=%x;slot=%x.\n",dev->device,privdata->boardID,privdata->iobase, privdata->irq, privdata->pci_slot  );
	return SUCCESS;
...
//     ULONG	curpage;
     ...
     advdrv_remove_device(&pcm3730i_driver, device);
...
static int __init pcm3730i_init(void)
{
     int error =0 ;
     struct semaphore *sema = NULL;
     sema = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
     init_MUTEX(sema);
     _ADV_SET_DRIVER_SEMA(pcm3730i_driver, sema);
     
     advdrv_register_driver( &pcm3730i_driver );
     error =  pci_module_init(&pci_driver) ;
     if( error != 0 )
     {
	  advdrv_unregister_driver( &pcm3730i_driver );
	  kfree(sema);
	  return error;
     }
     return SUCCESS;
...
static void __exit pcm3730i_exit(void)
{
     struct semaphore *sema = NULL;
     pci_unregister_driver( &pci_driver );
     advdrv_unregister_driver( &pcm3730i_driver );
     
     _ADV_GET_DRIVER_SEMA(pcm3730i_driver, sema);
     
     kfree(sema);
}

module_init( pcm3730i_init );
module_exit( pcm3730i_exit );

MODULE_DEVICE_TABLE(pci, serial_pci_tbl);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Advantech PCM3730I device driver module");
