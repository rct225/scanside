...
/* PCI-1753 driver object */
adv_driver pci1753_driver =
{
     .driver_name		= "pci1753",
     .devfs_name		= "pci1753",
     .ops			= &dev1753_fops,
...
static void __devexit advdrv_remove_one(struct pci_dev *dev)
{
     ...
     advdrv_remove_device(&pci1753_driver, device);
...
/**
 * pci1753_init - module initialize function
 * @void
 */
static INT32S __init pci1753_init(void)
{
	INT32S error =0 ;
	...
	init_MUTEX(sema);
	_ADV_SET_DRIVER_SEMA(pci1753_driver, sema);

	advdrv_register_driver( &pci1753_driver );
	error =  pci_register_driver(&pci_driver) ;
	if( error < 0 )
	{
		advdrv_unregister_driver( &pci1753_driver );
		kfree(sema);
		return error;
	}
...
/**
 * pci1753_exit - module exit callback function
 * @void
 */
static void __exit pci1753_exit(void)
{
	struct semaphore *sema = NULL;
	pci_unregister_driver( &pci_driver );
	advdrv_unregister_driver( &pci1753_driver );
     
	_ADV_GET_DRIVER_SEMA(pci1753_driver, sema);
	kfree(sema);
}

module_init( pci1753_init );
module_exit( pci1753_exit );

MODULE_DEVICE_TABLE(pci, serial_pci_tbl);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Advantech PCI1753 device driver module");
