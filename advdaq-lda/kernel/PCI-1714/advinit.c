 ...
 * 	advinit.c                                                 
 * Abstract:                                                         
 * 	Initialize the PCI-1714 driver module and device.         
 *                                                                         
 * Version history		                                          
...
/* PCI-1714 driver object */
adv_driver pci1714_driver = {
	.driver_name	= "pci1714",
	.devfs_name	= "pci1714",
	.ops		= &dev1714_fops,
};
...
{     
	...
	privdata->device_type = (dev->subsystem_device << 16) | (dev->device);
	privdata->board_id = (INT16U) (advInp(privdata, 0x2c) & 0x0f);
	
	
	/* support multi-card */
	switch (privdata->device_type) {
	case PCI1714U:
		privdata->timer_clock = 60;
		advdrv_device_set_devname(device, "pci1714U");
		break;
	case PCI1714UL_A:
	case PCI1714UL_B:
		privdata->timer_clock = 20;
		privdata->device_type = PCI1714UL;
		advdrv_device_set_devname(device, "pci1714UL");
		break;
	case PCI1714:
	default:
		privdata->timer_clock = 60;
		advdrv_device_set_devname(device, "pci1714");
		break;
	}
	...
	/* request I/O regions */
 	if (request_region(privdata->ioDMAbase, privdata->ioDMAlength, "PCI-1714") == NULL) {
		kfree(device);
		kfree(privdata);
 	...
 	if (request_region(privdata->iobase, privdata->iolength, "PCI-1714") == NULL) {
	...
	/* request irq */
	ret = request_irq(privdata->irq, pci1714_interrupt_handler, SA_SHIRQ, "adv1714", privdata);
	if (ret != 0) {
		release_region(privdata->ioDMAbase, privdata->ioDMAlength);
	...
	ret = advdrv_add_device(&pci1714_driver, device);
	if (ret) {
		release_region(privdata->ioDMAbase, privdata->ioDMAlength);
		release_region(privdata->iobase, privdata->iolength);
...
static void __devexit advdrv_remove_one(struct pci_dev *dev)
{
	...
	advdrv_remove_device(&pci1714_driver, device);
...
/**
 * pci1714_init - The module initialize PCI-1714 driver
 *
 * Enumerate all PCI cards on the bus, register the driver in Advantech core driver.
 */
static INT32S __init pci1714_init(void)
{
	INT32S error;
	...
	init_MUTEX(sema);
	_ADV_SET_DRIVER_SEMA(pci1714_driver, sema);


	advdrv_register_driver(&pci1714_driver);
	error = pci_module_init(&pci_driver);
	if (error != 0) {
		advdrv_unregister_driver(&pci1714_driver);
		return error;
	}
...
/**
 * pci1714_exit - Exit PCI-1714 driver
 */
static void __exit pci1714_exit(void)
{
	struct semaphore *sema = NULL;
	...
	advdrv_unregister_driver(&pci1714_driver);
	_ADV_GET_DRIVER_SEMA(pci1714_driver, sema);
     
	kfree(sema);
}

module_init(pci1714_init);
module_exit(pci1714_exit);

MODULE_DEVICE_TABLE(pci, serial_pci_tbl);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Advantech PCI1714 device driver module");
