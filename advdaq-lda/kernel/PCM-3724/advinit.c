 ...
 * 	advinit.c                                                 
 * Abstract:                                                         
 * 	Initialize the PCM-3724 driver module and device.         
 *                                                                         
 * Version history		                                          
...
static INT32S adv_opendevice(adv_device *device)
{
	...
	ret = adv_process_info_add(&priv->ptr_process_info, 2);
...
static INT32S adv_get_devinfo(adv_device *device, adv_devinfo *udev_info)
{
	...
	kdev_info.irq_ext = priv->irq_ext;
...
static adv_general_ops adv_dev3724_fops = {
...
static adv_driver pcm3724_adv_driver = {
	.owner = THIS_MODULE,
	.driver_name = DRV_NAME,
	.devfs_name = DRV_NAME,
	.ops = &adv_dev3724_fops,
};
	
static INT32S  pcm3724_isa_probe(struct adv_isa_device *idev)
{
	adv_device *dev = NULL;
	...
	dev->irq = idev->irq;
	dev->irq_ext = idev->irq_ext;
	priv->iobase = dev->iobase;
	priv->iolength = PCM3724_TOTAL_SIZE;
	priv->device_type = PCM3724;
	priv->irq = dev->irq;
	priv->irq_ext = dev->irq_ext;
...
/* 	} */
	...
	ret = request_irq(priv->irq, pcm3724_interrupt_handler00,
			  SA_INTERRUPT, idev->device_name, priv);
	if (ret) {
	...
	ret = request_irq(priv->irq_ext, pcm3724_interrupt_handler01,
			  SA_INTERRUPT, idev->device_name, priv);
	if (ret) {
		printk(KERN_ERR PFX "Could not allocate IRQ%d\n", priv->irq_ext);
		release_region(priv->iobase, priv->iolength);
	...
	ret = advdrv_add_device(&pcm3724_adv_driver, dev);
	if (ret) {
		printk(KERN_ERR PFX 
		       "Failed to add device into adv_driver object.\n");
		release_region(priv->iobase, priv->iolength);
		free_irq(priv->irq, priv);
		free_irq(priv->irq_ext, priv);
		free_advdev(dev);
		return ret;
	}
...
static INT32S pcm3724_device_remove(struct adv_isa_device *idev)
{
	...
	release_region(priv->iobase, PCM3724_TOTAL_SIZE);
	free_irq(priv->irq, priv);
	free_irq(priv->irq_ext, priv);
	advdrv_remove_device(&pcm3724_adv_driver, dev);
	free_advdev(dev);	
	
	return 0;
}

static struct adv_isa_driver pcm3724_isa_driver = {
		.name = DRV_NAME,
		.probe = pcm3724_isa_probe,
		.remove = pcm3724_device_remove,
};



/**
 * pcm3724 - The module initialize PCM-3724 driver
 * 
 * Loadable module initialization stuff, register the driver in Advantech 
 * core driver.
 */
static INT32S __init pcm3724_init(void)
{
	...
	init_MUTEX(sema);
	_ADV_SET_DRIVER_SEMA(pcm3724_adv_driver, sema);
	
	ret = adv_isa_register_driver(&pcm3724_isa_driver);
        ret |= advdrv_register_driver(&pcm3724_adv_driver);

	return ret;
}

/**
 * pcm3724_exit - Exit PCM-3724 driver
 */
static void __exit pcm3724_exit(void)
{
	struct semaphore *sema = NULL;
	
	_ADV_GET_DRIVER_SEMA(pcm3724_adv_driver, sema);
	kfree(sema);
	advdrv_unregister_driver(&pcm3724_adv_driver);
/* 	pcm3724_device_remove(device); */
	
	adv_isa_unregister_driver(&pcm3724_isa_driver);
	
	
	/* release_region(iobase, PCM3724_TOTAL_SIZE); */
/* 	free_irq(irq, priv); */
/* 	kfree(privdata); */
}

module_init(pcm3724_init);
module_exit(pcm3724_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("li.ang <li.ang@advantech.com.cn>");
MODULE_DESCRIPTION("PCM-3724 ISA device driver");
