 ...
 * 			Advantech Co., Ltd.                        
 *                                                                  
 * 	    Advantech PCI-1716 Series Device Driver for Linux             
 *                                                                  
 *                                                                  
 *                                                                  
 * 	File Name:                                                 
...
/**
 * pci1716_gp_ao - writes a binary value to one of the analog   
 ...
 *                                                            
 */
static INT32S pci1716_gp_ao(adv_device *device, INT16U chan, INT16U bin_data)
{
	...
	aoref |= mask;
	advOutp(privdata, 0x0e + chan, aoref);
...
INT32S adv_ao_binary_out(adv_device *device, void *arg)
{
	...
	case PCI1716:
		if (ao_binary_out.chan > 1)
			...
			ret = pci1716_gp_ao(device, ao_binary_out.chan, ao_binary_out.BinData);
		break;
	default:
		return -EFAULT;
	}
...
INT32S adv_ao_voltage_out(adv_device *device, void *arg)
{
	...
	switch (privdata->device_type) {
	case PCI1716:
		ret = pci1716_gp_ao(device, chan, wdata);
		break;
	default:
		return -EFAULT;
	}
