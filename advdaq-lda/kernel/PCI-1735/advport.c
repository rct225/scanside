 ...
 *                                                                  
 * 	Version history                                            
 * 	10/18/2006			Create by zhenyu.zhang           
 *                                                                  
 ********************************************************************/
#include "PCI1735.h"
...
INT32S adv_read_port(adv_device *device, void *arg)
{
	...
	case AD_READ_PORT_UCHAR:
		*(INT8U *) buf = advInp(privdata, port_num)&0xff;
		ret_len = sizeof(INT8U);
		break;
	case AD_READ_PORT_USHORT:
		*(INT16U *) buf = advInpw(privdata, port_num)&0xffff;
		ret_len = sizeof(INT16U);
		break;
	case AD_READ_PORT_ULONG:
		*(INT32U *) buf = advInpdw(privdata, port_num)&0xffffffff;
