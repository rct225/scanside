 ...
 * 	advinit.c                                                 
 * Abstract:                                                         
 * 	Initialize the USB-4711 driver module and device.         
 *                                                                         
 * Version history		                                          
...
/* USB-4711 driver object */
adv_driver usb4711_driver = {
	.driver_name	= "usb4711",
	.devfs_name	= "usb4711",
	.ops		= &usb4711_fops,
};
...
static void * advdrv_init_one(struct usb_device *dev, unsigned intf, const struct usb_device_id *id)
{
	...
	case USB4711A:
		...
		advdrv_device_set_devname(device, "usb4711");
		break;
	case USB4711B:
		privdata->board_id &= 0x0f;
		advdrv_device_set_devname(device, "usb4711a");
		break;
	default:
		kfree(urb_sema);
		kfree(device);
	...
	ret = advdrv_add_device(&usb4711_driver, device);
	if (ret) {
		kfree(urb_sema);
		kfree(device);
		kfree(privdata);
	...
	KdPrint("Add a USB-4711 device: type=%xh; boardID=%xh.\n",
...
static INT32S __devinit advdrv_init_one(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	/* get USB speed (low, full or high) */
	ret = usb_string(privdata->udev, privdata->udev->descriptor.iProduct,
	...
	if (strstr(strbuf, "(LS)") != NULL) {
		privdata->usb_speed = USB_SPEED_LOW;
	} else if (strstr(strbuf, "(FS)") != NULL) {
		privdata->usb_speed = USB_SPEED_FULL;
	} else if (strstr(strbuf, "(HS)") != NULL) {
		privdata->usb_speed = USB_SPEED_HIGH;
	}
	...
	spin_lock_init(&privdata->spinlock);
	init_waitqueue_head(&privdata->event_wait);
	tasklet_init(&privdata->urb_tasklet, urb_tasklet_fn, (PTR_T) device);
	INIT_WORK(&privdata->fai_stop_work, adv_fai_stop_work, (VOID *) device);
...
static void adv_device_delete(adv_device *device)
{
	...
	KdPrint("Remove a USB-4711 device\n" );
	...
	advdrv_remove_device(&usb4711_driver, device);
...
/**
 * usb4711_init - The module initialize USB-4711 driver
 *
 * Enumerate all PCI cards on the bus, register the driver in Advantech core driver.
 */
static INT32S __init usb4711_init(void)
{
	int error = 0;
	...
	_ADV_SET_DRIVER_SEMA(usb4711_driver, sema);
	advdrv_register_driver(&usb4711_driver);
	
	error = usb_register(&usb_driver);
	if (error != 0) {
		advdrv_unregister_driver(&usb4711_driver);
		return error;
	}
...
/**
 * pci1716_exit - Exit USB-4711 driver
 */
static void __exit usb4711_exit(void)
{
	struct semaphore *sema = NULL;


	usb_deregister(&usb_driver);
	if (advdrv_unregister_driver(&usb4711_driver)) {
		return;
	}
	
	_ADV_GET_DRIVER_SEMA(usb4711_driver, sema);
	
	kfree(sema);
}

module_init(usb4711_init);
module_exit(usb4711_exit);

MODULE_DEVICE_TABLE(usb, serial_usb_tbl);
MODULE_AUTHOR("Li Ang <li.ang@advantech.com.cn>");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Advantech USB4711 device driver module");
